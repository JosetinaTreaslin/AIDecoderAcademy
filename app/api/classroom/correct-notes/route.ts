...
export async function POST(req: Request) {
  try {
    ...
    if (subject === "Kannada") {
      const sarvamResult = await runSarvamHybrid(image_urls, chapter_title, content_text);

      if (sarvamResult) {
        parsed = sarvamResult;
      } else {
        // ⚠️ Sarvam failed → fall back to Gemini vision
        console.warn("[correct-notes] Falling back to Gemini vision for Kannada");
    ...
    if (subject === "Kannada" && issues.length > 0) {
      try {
        ...
        console.error("[correct-notes] vision bbox (non-fatal):", bboxErr.message);
      }
    }
