...
export default function SignInPage() {
  ...
  const handleForgotEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError((err as { errors?: { message: string }[] })?.errors?.[0]?.message ?? "Could not send reset email. Please check the address and try again.");
    } finally { setLoading(false); }
  };
  ...
  const handleForgotCode = async (e: React.FormEvent) => {
    ...
    if (!isLoaded) return;
    setError("");
    if (!/^\d{6}$/.test(resetCode.trim())) { setError("Please enter the 6-digit code sent to your email."); return; }
    setLoading(true);
    try {
      const result = await signIn.attemptFirstFactor({ strategy: "reset_password_email_code", code: resetCode.trim() } as Parameters<typeof signIn.attemptFirstFactor>[0]);
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError((err as { errors?: { message: string }[] })?.errors?.[0]?.message ?? "Something went wrong. Please try again.");
    } finally { setLoading(false); }
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, overflow: "hidden" }}>
      ...
      {/* Top-right nav — same %-of-viewport coordinate system as BgFixed and the form card,
          so background image + card + nav move and resize together as one fixed composition. */}
      ...
      {/* Form card — same percentage offsets as the sign-up card, so the live card and the
          background image move/resize together as a single unit at any screen size. */}
          ...
          <div style={{ padding: "1.6vw 2.13vw 1.87vw" }}>
            ...
            {forgotMode && resetStep === "email" && (
              <form onSubmit={handleForgotEmail} style={{ display: "flex", flexDirection: "column", gap: "1.07vw" }}>
                <button type="button" onClick={() => { setForgotMode(false); setResetStep("email"); setError(""); }}
                  style={{ alignSelf: "flex-start", fontSize: "0.8vw", fontWeight: 600, color: LBLUE, background: "none", border: "none", cursor: "pointer" }}>
                  ← Back to sign in
                </button>
                <h3 style={{ fontFamily: "var(--font-syne), system-ui, sans-serif", fontWeight: 800, fontSize: "1.2vw", color: NAVY }}>Reset your password</h3>
                <p style={{ fontSize: "0.87vw", color: "#374151" }}>We&apos;ll email you a 6-digit code to reset your password.</p>
            ...
            {forgotMode && resetStep === "code" && (
              <form onSubmit={handleForgotCode} style={{ display: "flex", flexDirection: "column", gap: "1.07vw" }}>
                ...
                <h3 style={{ fontFamily: "var(--font-syne), system-ui, sans-serif", fontWeight: 800, fontSize: "1.2vw", color: NAVY }}>Enter the code</h3>
                <p style={{ fontSize: "0.87vw", color: "#374151" }}>
                  We sent a 6-digit code to <strong style={{ color: LBLUE }}>{email}</strong>. Enter it below.
                </p>
                <input type="text" value={resetCode} onChange={e => setResetCode(e.target.value.replace(/\D/g, "").slice(0,6))}
                  ...
                  style={{ width: "100%", boxSizing: "border-box", textAlign: "center", letterSpacing: "0.3em", fontSize: "1.2vw", fontWeight: 800, padding: "0.73vw", background: IBKG, border: `1px solid ${BORDER}`, borderRadius: "0.67vw", outline: "none", color: NAVY }}
                  onFocus={e => (e.currentTarget.style.borderColor = BLUE)}
                  onBlur={e  => (e.currentTarget.style.borderColor = BORDER)}
                />
                {error && <p style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", borderRadius: "0.67vw", padding: "0.67vw 0.93vw", fontSize: "0.87vw" }}>{error}</p>}
            ...
            {forgotMode && resetStep === "password" && (
              <form onSubmit={handleForgotPassword} style={{ display: "flex", flexDirection: "column", gap: "1.07vw" }}>
                ...
                <h3 style={{ fontFamily: "var(--font-syne), system-ui, sans-serif", fontWeight: 800, fontSize: "1.2vw", color: NAVY }}>Set new password</h3>

                <ul style={{ display: "flex", flexDirection: "column", gap: "0.27vw", listStyle: "none", padding: 0, margin: 0 }}>
                  {passReqs.map(r => (
                ...
                <Inp label="New password" icon={<LockIcon/>}
                  type={showNewPw ? "text" : "password"} value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="New password"
                  rightIcon={showNewPw ? <EyeOffIcon/> : <EyeIcon/>} onRightClick={() => setShowNewPw(v => !v)}
                />
                <Inp label="Confirm password" icon={<LockIcon/>}
                  type={showConfirmPw ? "text" : "password"} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Confirm password"
                  rightIcon={showConfirmPw ? <EyeOffIcon/> : <EyeIcon/>} onRightClick={() => setShowConfirmPw(v => !v)}
                />
                {confirmPassword && (
                  <p style={{ fontSize: "0.8vw", fontWeight: 600, color: newPassword === confirmPassword ? "#16a34a" : "#dc2626" }}>
                    {newPassword === confirmPassword ? "✓ Passwords match" : "✗ Passwords do not match"}
                  </p>
                )}
                {error && <p style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", borderRadius: "0.67vw", padding: "0.67vw 0.93vw", fontSize: "0.87vw" }}>{error}</p>}
                <button type="submit" disabled={loading || newPassword !== confirmPassword || passReqs.some(r => !r.ok)}
                  style={{ width: "100%", padding: "1vw", background: BLUE, color: "#fff", borderRadius: 999, fontSize: "1vw", fontWeight: 700, border: `1.5px solid ${GOLD}`, cursor: "pointer", opacity: (loading || newPassword !== confirmPassword || passReqs.some(r => !r.ok)) ? 0.4 : 1, boxShadow: "0 4px 20px rgba(29,78,216,0.4), 0 0 12px rgba(200,160,80,0.35)" }}>
                ...
                <div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.4vw" }}>
                    ...
                    <button type="button" onClick={() => { setForgotMode(true); setError(""); }}
                      style={{ fontSize: "0.8vw", fontWeight: 600, color: LBLUE, background: "none", border: "none", cursor: "pointer" }}>
                      Forgot password?
                    </button>
                  </div>
                  <div style={{ position: "relative" }}>
