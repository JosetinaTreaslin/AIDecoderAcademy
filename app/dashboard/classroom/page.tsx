...
export default function ClassroomPage() {
  ...
  // ── Proctoring: disqualify and force-zero the active test ──────────────────
  const handleDisqualify = useCallback(() => {
    if (!paper) return;

    if (paper.type === "mcq") {
      const questions = paper.questions as MCQQuestion[];
      const feedback: MCQResult["feedback"] = {};
      for (const q of questions) {
        feedback[q.id] = {
          correct:       false,
          correct_index: -1,
          explanation:   "Test terminated: proctoring violation (left fullscreen or switched tabs).",
        };
      }
      setMcqResult({ score: 0, max_score: questions.length, feedback, questions, answers: {} });
      setView("mcq-result");
    } else {
      const questions = paper.questions as WrittenQuestion[];
      const feedback: Record<string, WrittenFeedbackItem> = {};
      for (const q of questions) {
        feedback[q.id] = {
          score:    0,
          max:      q.marks,
          feedback: "Test terminated: proctoring violation (left fullscreen or switched tabs).",
        };
      }
      setWrittenResult({
        score:     0,
        max_score: questions.reduce((s, q) => s + q.marks, 0),
        feedback,
        questions,
      });
  ...
  return (
    <div className="flex flex-col" style={{ height: "100dvh", background: "#08080F" }}>
      ...
      {/* Content — ProctoringGuard wraps everything, active only during tests */}
